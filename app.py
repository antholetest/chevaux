...
def synchroniser_github_api(filename_str, content_bytes, message, token=None):
    if not token:
...
def supprimer_github_api(filename_str, message="Suppression fichier", token=None):
    if not token:
        return
...
def sauvegarder_et_synchroniser(
    ...
    threading.Thread(
        ...
        args=(filename_path.name, content_bytes, message, token),
    ).start()
...
def charger_modele_ia():
    if not FICHIER_MODELE_IA.exists():
        sauvegarder_et_synchroniser(
            MODELE_IA_DEFAUT, FICHIER_MODELE_IA, "Initialisation du modèle IA"
        )
        ...
        for f in DOSSIER.glob(pattern):
            try:
                ...
                threading.Thread(
                    ...
                    args=(filename_str, "Remise à zéro admin", token),
                ).start()
            except Exception:
                pass
...
def telecharger_pmu_date(date_iso, fichier_cible):
    ...
    sauvegarder_et_synchroniser(
        resultats_journee, fichier_cible, f"Téléchargement courses {date_iso}"
    )
    st.cache_data.clear()
    return True
...
def retroaction_apprentissage_ia(
    ...
    if statut == "Gagné":
        ...
        modele_ia["stats_impact"]["gain_cumule_ia"] = round(
            modele_ia["stats_impact"].get("gain_cumule_ia", 0.0) + profit_net, 2
        )
        ...
        modele_ia["stats_impact"]["gain_cumule_ia"] = round(
            modele_ia["stats_impact"].get("gain_cumule_ia", 0.0) - mise_totale, 2
        )
...
def generer_et_sauvegarder_bilan_journee(historique, date_str):
    historique_jour = [
        p
        for p in historique
        if str(p.get("date")) == str(date_str) and p.get("statut") != "Annulé"
    ]
    ...
    for p in historique_jour:
        ...
        if not reunion_nom or reunion_nom == "Inconnu" or (reunion_nom.startswith("R") and len(reunion_nom) <= 3):
            ...
            m_rc = re.match(r"^(.*?)\s*-\s*(C\d+)", rc_full)
            if m_rc:
                reunion_nom = m_rc.group(1).strip()
            elif " - " in rc_full:
                reunion_nom = rc_full.rsplit(" - ", 1)[0].strip()
...
def verifier_resultats_automatiques_pmu(historique):
    ...
    for p in historique:
        if p.get("statut") == "En attente":
            ...
            if not c_match:
                c_match = re.search(
                    r"\b(\d+)(?:ère|ème|e)?\s*course\b", texte_global, re.IGNORECASE
                )
            ...
            try:
                ...
                for part in liste_partants_bruts:
                    ...
                    if isinstance(rapport, dict) and isinstance(
                        rapport.get("rapport"), (int, float)
                    ):
                ...
                for part in parts:
                    ...
                    mise_part = (
                        float(mise_part_m.group(1).replace(",", "."))
                        if mise_part_m
                        else (mise_totale / len(parts))
                    )

                    is_place = (
                        "placé" in part_lower
                        or "place" in part_lower
                        or "sécu" in part_lower
                    )
                    is_gagnant = (
                        "gagnant" in part_lower
                        or "poker" in part_lower
                        or "spéculatif" in part_lower
                    )
                    ...
                    if is_place and nums_part:
                        ...
                        if div_ref == 0 and cote_ref > 1.0:
                            div_ref = max(
                                ...
                                1.0
                                + (cote_ref - 1.0)
                                / (3.6 if len(liste_partants_bruts) >= 8 else 2.5),
                            )
                        ...
                        div_ref = dividendes_officiels.get(
                            ("SIMPLE_GAGNANT", num_poker), cote_ref
                        )
                ...
                p["diagnostic"] = retroaction_apprentissage_ia(
                    p, arrivee_trouvee, cotes_reelles, liste_partants_bruts
                )
                ...
                print(f"Erreur vérification pari : {e}")
...
with st.sidebar.expander("🛠️ Administration et réinitialisation"):
    ...
    if st.button("🧹 Vider le cache Streamlit"):
        st.cache_data.clear()
        st.success("Cache Streamlit vidé avec succès !")

    st.divider()
    st.write("🤖 **Nourrir l'IA :** Parier sur toutes les courses (Seuil EV >= 1.50)")
    if st.button("🚀 Simuler le Chrono (10€/course)"):
        if mdp_admin.strip() == st.secrets.get("PASSWORD", "301180"):
            date_iso = st.session_state.get(
                "date_commune", datetime.date.today()
            ).strftime("%Y-%m-%d")
            ...
            if fichier_jour.exists():
                ...
                if FICHIER_HISTORIQUE.exists():
                    with open(
                        FICHIER_HISTORIQUE, "r", encoding="utf-8", errors="replace"
                    ) as f:
                        try:
                            hist = json.load(f)
                        except Exception:
                ...
                for c_elem in donnees_chrono:
                    if not c_elem.get("nom_course") or len(
                        str(c_elem.get("nom_course"))
                    ) <= 2:
                        continue
                    ...
                    if chevaux_val_c:
                        for c in chevaux_val_c:
                            c["score_analyse"] = evaluer_score_cheval(
                                ...
                                date_iso,
                                params_ad_chrono,
                                distance_course=c_elem.get("distance", ""),
                            )
                        normaliser_scores_chevaux(chevaux_val_c, "score_analyse")
                        calculer_valeur_esperee_avancee(
                            chevaux_val_c, len(c_elem.get("chevaux", []))
                        )

                        chevaux_val_c.sort(
                            key=lambda x: (x.get("ev_index", 0), x["score_analyse"]),
                            reverse=True,
                        )
                        ...
                        if base_chev.get("ev_index", 0) >= 1.50:
                            ...
                            poker_chev = (
                                max(outsiders_c, key=lambda x: x.get("ev_index", 0))
                                if outsiders_c
                                else (
                                    chevaux_val_c[1]
                                    if len(chevaux_val_c) > 1
                                    else base_chev
                                )
                            )
                            ...
                            hist.append({
                                ...
                                "details": (
                                    f"Simple Placé (Sécurité Value) ➔ N°{base_chev.get('num', '?')}"
                                    f" (8.0€) | Simple Gagnant (Poker Value) ➔"
                                    f" N°{poker_chev.get('num', '?')} (2.0€)"
                                ),
                                "mise": 10.0,
                                "statut": "En attente",
                                "gain": 0.0,
                ...
                if paris_ajoutes > 0:
                    sauvegarder_et_synchroniser(
                        hist,
                        FICHIER_HISTORIQUE,
                        f"Ajout de {paris_ajoutes} paris apprentissage IA",
                    )
                    ...
                    st.warning("Aucune course à EV >= 1.50 aujourd'hui.")
            else:
                st.error("Aucune donnée trouvée pour cette date.")
        else:
            st.error("Mot de passe admin incorrect.")
...
with tab_chronologique:
    ...
    if verifier_stop_loss(date_chrono_iso):
        st.error(
            "⚠️ **Alerte Stop-Loss Déclenché :** Les pertes cumulées dépassent"
            " 35.00 € aujourd'hui."
        )
    ...
    if fichier_chrono_jour.exists():
        ...
        for c_elem in donnees_chrono:
            ...
            base_chev, poker_chev = {
                "num": "?",
                "nom": "Inconnu",
                "cote": 0.0,
                "ev_index": 0.0,
            }, {"num": "?", "nom": "Inconnu", "cote": 0.0, "ev_index": 0.0}
            if chevaux_val_c:
                params_ad_chrono = calculer_parametres_adaptatifs()
                for c in chevaux_val_c:
                    c["score_analyse"] = evaluer_score_cheval(
                ...
                poker_chev = (
                    ...
                    if outsiders_c
                        ...
                        chevaux_val_c[1] if len(chevaux_val_c) > 1 else base_chev
                    )
                )
        ...
        for idx_c, item_c in enumerate(toutes_courses):
            ...
            with st.expander(
                f"🕒 {item_c['heure']} | {item_c['reunion']} ➔"
                f" {item_c['course_num']} : {item_c['nom_course']}"
            ):
                st.markdown(
                    f"**Base Value Bet (Sécurité) :** Simple Placé ➔ N°{b_chev.get('num')}"
                    f" - {b_chev.get('nom')} (Cote Placé:"
                    f" {b_chev.get('cote_place_estimee', 0):.1f} | **EV Placé:"
                    f" {b_chev.get('ev_place_index', 0):.2f}**) "
                )
                st.markdown(
                    f"**Coup de Poker Value :** Simple Gagnant ➔ N°{p_chev.get('num')}"
                    f" - {p_chev.get('nom')} (Cote: {safe_float(p_chev.get('cote')):.1f}"
                    f" | **EV: {p_chev.get('ev_index', 0):.2f}**) "
                )
                ...
                with col_m:
                    ...
                    st.caption(
                        f"💡 Répartition Sécurisée : **{mise_secu} €** Sécu (80%) |"
                        f" **{mise_poker} €** Poker (20%)"
                    )
                ...
                with col_b:
                    ...
                    if st.button("⚡ Valider & Enregistrer", key=f"btn_valider_{cle_unique_course}"):
                        ...
                        nouveau_pari = {
                            ...
                            "type": "Rapide Value",
                            "details": (
                                f"Simple Placé (Sécurité Value) ➔ N°{b_num} ({mise_secu}€) |"
                                f" Simple Gagnant (Poker Value) ➔ N°{p_num} ({mise_poker}€)"
                            ),
                        ...
                        sauvegarder_et_synchroniser(
                            hist, FICHIER_HISTORIQUE, "Pari rapide enregistré"
                        )
...
with tab_analyse:
    ...
    if fichier_jour.exists():
        ...
        if reunions_map:
            reunion_choisie = st.selectbox(
                "Réunion", sorted(list(reunions_map.keys()))
            )
            ...
            st.info(
                f"Terrain : {course_curr.get('terrain_officiel')} | Discipline :"
                f" {course_curr.get('discipline')}"
            )
            ...
            if st.button("⚡ Lancer l'Analyse Maximisation Gains IA"):
                ...
                for c in course_curr.get("chevaux", []):
                    c["score_ia"] = evaluer_score_cheval(
                        ...
                        date_iso,
                        params,
                        distance_course=course_curr.get("distance", ""),
                    )
                normaliser_scores_chevaux(course_curr.get("chevaux", []), "score_ia")
                calculer_valeur_esperee_avancee(
                    course_curr.get("chevaux", []), len(course_curr.get("chevaux", []))
                )

                chevaux_tries = sorted(
                    course_curr.get("chevaux", []),
                    key=lambda x: x.get("ev_index", 0),
                    reverse=True,
                )
                st.dataframe(
                    [{
                        ...
                        "Driver": c["driver"],
                        "Cote": c.get("cote"),
                        "Score IA": c.get("score_ia"),
                        "Proba (Softmax)": (
                            f"{safe_float(c.get('proba_estimee',0))*100:.1f}%"
                        ),
        ...
        st.divider()
        st.subheader(
            "💰 Allocation Stratégique (Critère de Kelly & Modèle Prudent)"
        )
        budget_saisi = st.number_input(
            "Budget Global à Allouer (€)",
            min_value=5,
            max_value=500,
            value=50,
            step=5,
        ...
        if st.button("🎲 Calculer le Plan d'Allocation Optimal"):
            ...
            plan = generer_plan_budget_journalier(
                fichier_jour, budget_saisi, params_ad, date_iso=date_iso
            )
            if plan:
                ...
                st.info(
                    "Aucune opportunité ne présente un EV >= 1.50 aujourd'hui."
                )
        ...
        if st.session_state.get("plan_courant"):
            ...
            if st.button("✅ Enregistrer tout ce plan de mise", type="primary"):
                ...
                for item in st.session_state["plan_courant"]:
                    ...
                    m_c = re.search(r"\b(C\d+)\b", r_c)
                    c_num = m_c.group(1) if m_c else "C?"
                    ...
                    pari_obj = {
                        ...
                        "details": (
                            f"Sécu ({item.get('Mise Sécu')}):"
                            f" {item.get('Base Value Bet (Sécurité)')} | Poker"
                            f" ({item.get('Mise Poker')}):"
                            f" {item.get('Coup de Poker Value')}"
                        ),
                        "mise": mise_tot,
                        "statut": "En attente",
                        "gain": 0.0,
                ...
                sauvegarder_et_synchroniser(
                    hist, FICHIER_HISTORIQUE, "Enregistrement plan budgétaire Value"
                )
...
# --- TAB 3 : DASHBOARD IA ---
with tab_ia:
    st.title("🧠 Moteur IA & Paramètres Figés")
    modele_ia = charger_modele_ia()
    ...
    col_k2.metric(
        "Seuil EV Min",
        f"{modele_ia.get('seuil_value_bet', 1.50):.2f}",
    )
    col_k3.metric(
        "Kelly Fraction", f"{modele_ia.get('frequence_kelly', 0.05):.2f}"
    )
    col_k4.metric(
        "Gain Cumulé IA",
        f"{modele_ia.get('stats_impact', {}).get('gain_cumule_ia', 0.0):+.2f} €",
    )

    st.divider()
    st.subheader("📊 Pondérations Actuelles (Figées pour accumulation de Dataset)")
    ...
    with col_g1:
        df_poids = pd.DataFrame([
            ...
            {"Critère": "Tendance Cotes", "Poids IA": modele_ia.get("poids_cote_tendance", 1.35)},
            {"Critère": "Impact Driver / Jockey", "Poids IA": modele_ia.get("poids_driver", 1.15)},
            {"Critère": "Détection Outsider", "Poids IA": modele_ia.get("poids_outsider_cache", 1.30)},
        ])
        st.bar_chart(df_poids.set_index("Critère"))

    with col_g2:
        st.info("📌 **Note d'entraînement :** Les poids dynamiques sont actuellement **figés** pour accumuler un jeu de données propre d'au moins 1 000 à 5 000 courses sans osciller sur le bruit à court terme.")
...
with tab_suivi:
    ...
    if FICHIER_HISTORIQUE.exists():
        ...
        with col_btn1:
            if st.button("🔄 Vérifier les résultats PMU"):
                with st.spinner("Analyse du rendement..."):
                    if verifier_resultats_automatiques_pmu(historique):
                        sauvegarder_et_synchroniser(
                            historique, FICHIER_HISTORIQUE, "Mise à jour résultats"
                        )
                        st.success("Résultats actualisés !")
                        st.rerun()
                    else:
        ...
        with col_btn2:
            with st.popover("🗑️ Réinitialiser les montants"):
                st.warning(
                    "⚠️ **Confirmation requise**\n\nCette action remet à zéro les"
                    " mises et gains enregistrés."
                )
                if st.button("🔥 Confirmer la remise à zéro", type="primary"):
                    for p in historique:
                        ...
                        p["gain"] = 0.0
                    sauvegarder_et_synchroniser(
                        historique, FICHIER_HISTORIQUE, "Remise à zéro montants"
                    )
        ...
        total_gain = sum(
            safe_float(p.get("gain", 0))
            for p in historique
            if p.get("statut") == "Gagné"
        )
        bilan_net = total_gain - total_mise
        roi_global = (
            ((total_gain - total_mise) / total_mise * 100) if total_mise > 0 else 0.0
        )
        ...
        if st.button("Lancer les tests de significativité"):
            ...
            if len(paris_evaluables) < 10:
                st.warning(f"Seulement {len(paris_evaluables)} paris terminés. Il en faut au moins 10 pour un test pertinent.")
            else:
